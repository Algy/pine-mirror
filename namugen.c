...
struct doc_itfc {
    bool (*doc_exists)(struct doc_itfc *, char *doc_name);
    sds (*doc_href)(struct doc_itfc *, char *doc_name);
};

struct gen_hook_itfc {
    bool (*hook_fn_link)(struct gen_hook_itfc *, struct namuast_ctx *ctx, struct namuast_ctx *inl, char *fn);
    bool (*hook_fn_call)(struct gen_hook_itfc *, struct namuast_ctx *ctx, struct namuast_ctx *inl, char *fn, char* arg);
};

struct heading {
    struct namuaset_inline *content;
    sds section_name;
...
struct namugen_ctx {
    ...
    struct doc_itfc *doc_itfc;
    struct gen_hook_itfc *gen_hook_itfc;
...
// steal raw_content if exists
static sds generate_link(sds buf, bool exist, char *href, char *section, char *title, char *pcontent, sds raw_content, char *extra_class, char *extra_attr) {
    sds content;
...
static void emit_fnt_item(struct namuast_inline* inl, struct fnt *fnt) {
    sds id_def = sdscatprintf(sdsnew(NULL), "id='#rfn-%d'", id);
    sds mark;
    sds href = sdscatprintf(sdsnew(NULL), "#fn-%d'", id);
    if (fnt->is_named) {
        sds val = escape_html_content(fnt->repr.name);
        mark = sdscatprintf(sdsnew(NULL), "[%s]", fnt->repr.name);
        sdsfree(val);
    } else {
        mark = sdscatprintf(sdsnew(NULL), "[%d]", fnt->repr.anon_num);
    }
    inl->buf = sdscat(inl->buf, "<span class='footnote-list'>");
    inl->buf = generate_link(inl->buf, true, href, NULL, "", NULL, mark, "wiki-rfn-content", id_def);
    inl->buf = sdscatsds(inl->buf, fnt->content->buf);
    inl->buf = sdscat(inl->buf, "</span>");
    sdsfree(mark);
    sdsfree(id_def);
    sdsfree(href);
}
...
static sds dfs_toc(sds buf, struct heading* hd) {
    if (hd->parent) { 
        // if it is not the root, which is a sentinel node
...
static void do_emit_toc(struct namugen_ctx* ctx) {
    if (ctx->toc_count > 0) {
        ...
        for (idx = 0; idx < ctx->toc_count; idx++) {
            ...
            sdscatlen(ctx->main_buf, pos - prev_position);
            new_buf = sdscatsds(new_buf, toc_buf);
            prev_position = pos;
        }
        new_buf = sdscatlen(new_buf, ctx->prev_position, ctx->main_buf + main_buf_len - prev_position);
...
static struct namugen_ctx* make_ctx(char* cur_doc_name, struct doc_itfc* doc_itfc, struct gen_hook_itfc* gen_hook_itfc) {
...
static sds ctx_flush_main_buf(struct namugen_ctx* ctx) {
...
static void remove_ctx(struct namugen_ctx* ctx) {
    if (ctx->main_buf)
        sdsfree(ctx->main_buf);
    int idx = 0;
...
void nm_emit_inline(struct namugen_ctx* ctx, struct namuast_inline* inl) {
    ctx->main_buf = sdscatsds( , inl->buf)
    remove_
}
...
void nm_emit_table(struct namugen_ctx* ctx, struct namuast_table* tbl) {
    ...
    sds tbl_style = sdsnew(NULL);
    sds tbl_attr = sdsnew(NULL);
    sds tbl_caption = sdsnew(NULL);
    ...
    for (idx = 0; idx < tbl->row_count; idx++) {
        ...
        for (kdx = 0; kdx < row->col_count; kdx++) {
            struct namuast_table_cell *cell = &tbl->cols[kdx];
            sds cell_attr = sdsnew(NULL);
            sds cell_style = sdsnew(NULL);

            char span[100];
            if (cell->row_span > 1) {
                sds n = sdsfromlonglong(cell->row_span);
                cell_attr = add_html_attr(cell_attr, "rowspan", n);
                sdsfree(n);
            }
            if (cell->col_span > 1) {
                sds n = sdsfromlonglong(cell->col_span);
...
void nm_inl_emit_link(struct namuast_inline* inl, char *link, bool compatible_mode, char *alias, char *section) {
    struct namugen_ctx ctx = inl->ctx;
    struct gen_hook_itfc* hook_itfc = ctx->hook_itfc;
    struct doc_itfc* doc_itfc = ctx->doc_itfc;
    ...
    // main process
    bool exists = doc_itfc->doc_exists(doc_itfc, link);
...
void nm_inl_emit_upper_link(struct namuast_inline* inl, char *alias, char *section) {
    sds cur_doc_name = inl->ctx->cur_doc_name; // borrowed
    ...
    inl->buf = generate_link(inl->buf, exist, href? href : "", section, upper_doc_name? upper_doc_name : "../", NULL, "wiki-internal-link", NULL);
    if (href)
        sdsfree(href);
    if (upper_doc_name)
...
void nm_inl_emit_lower_link(struct namuast_inline* inl, char *link, char *alias, char *section) {
    ...
    bool exists = inl->ctx->doc_itfc->doc_exists(doc_itfc, docname);
    inl->buf = generate_link(inl->buf, exist, href, section, link, alias? alias : link, "wiki-internal-link", NULL);
...
void nm_inl_emit_external_link(struct namuast_inline* inl, char *link, char *alias) {
    inl->buf = generate_link(inl->buf, true, link, NULL, alias? alias : link, "wiki-extenral-link", NULL);
    if (link)
        free(link);
    if (alias)
        free(alias);
...
void nm_inl_emit_image(struct namuast_inline* inl, char *url, char *width, char *height, int align) {
    ...
    if (align != nm_align_none) {
        ...
        if (!strcmp(align_repr, "center"))
            align_repr = "middle";
        image = sdsconcatprintf(image, "style='text-align: %s;'", align_repr);
    }
    image = sdscat(image, ">");
...
void nm_inl_emit_footnote_mark(struct namuast_inline* inl, int id, struct namugen_ctx *ctx) {
    ...
    sds id_def = sdscatprintf(sdsnew(NULL), "id='#fn-%d'", id);
    sds href = sdscatprintf(sdsnew(NULL), "#rfn-%d'", id);
    sds mark;
    ...
    inl->buf = generate_link(inl->buf, true, href, NULL, fnt->content->buf, NULL, mark, "wiki-fn-content", id_def);
    sdsfree(mark);
    sdsfree(id_def);
    sdsfree(href);
}
